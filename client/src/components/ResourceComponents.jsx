  ...
  return (
    <section className="flex flex-col gap-2">
      <label htmlFor="CPU" className="flex flex-col w-full">
        CPU
        <select name="CPU" id="CPU" className="border-2 rounded-md p-2 w-full">
          {componentData
            ?.filter((cmp) => cmp.type === "CPU")
            .map((cmp) => {
              return (
                <option label={cmp.name} value={cmp._id} key={cmp._id}>
                  {cmp.name}
                </option>
              );
            })}
      ...
      <label htmlFor="GPU" className="flex flex-col w-full">
        GPU
        <select name="GPU" id="GPU" className="border-2 rounded-md p-2 w-full">
          {componentData
      ...
      <label htmlFor="RAM" className="flex flex-col w-full">
        RAM
        <select name="RAM" id="RAM" className="border-2 rounded-md p-2 w-full">
          {componentData
      ...
      <label htmlFor="SSD" className="flex flex-col w-full">
        SSD
        <select name="SSD" id="SSD" className="border-2 rounded-md p-2 w-full">
          {componentData
