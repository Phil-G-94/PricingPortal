  ...
  return (
    <section className="flex flex-col gap-2">
      <label htmlFor="chassis" className="flex flex-col w-full">
        Chassis
        <select name="chassis" id="chassis" required className="border-2 rounded-md p-2 w-full">
          {componentData
            ?.filter((cmp) => cmp.type === "chassis")
            .map((cmp) => {
              return (
                <option key={cmp._id} label={cmp.name} value={cmp._id}>
                  {cmp.name}
                </option>
              );
            })}
      ...
      {componentData
        ...
        .map((cmp) => {
          return (
            <label key={cmp._id} htmlFor={`coolingCabling_${cmp._id}`} className="hidden">
              Cooling/Cabling
              <input
                name="coolingCabling"
      ...
      {componentData
        ...
        .map((cmp) => {
          return (
            <label key={cmp._id} htmlFor={`islc_${cmp._id}`} className="hidden">
              ISLC
              <input
                name="islc"
                id={`islc_${cmp._id}`}
